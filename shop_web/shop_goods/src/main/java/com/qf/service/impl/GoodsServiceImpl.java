...
//        itemFeign.createHtml(goods);
        ...
        if (goods.getType() == 2){
            ...
            //计算评分
            double score = TimeUtil.dateSCore(goods.getGoodsSeckill().getStartTime());
            //将商品信息保存到redis
            stringRedisTemplate.opsForZSet().add( ContactUtil.REDIS_SECKILL_SORT_SET, goods.getId() + "", score);
